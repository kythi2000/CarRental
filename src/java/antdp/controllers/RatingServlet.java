...
public class RatingServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "GetHistoryRentalServlet";
        ...
        RatingDTO ratingDTO = new RatingDTO(carID, email, comment.trim(), Integer.parseInt(point));
        try {
            if (ratingDAO.insert(ratingDTO)) {
                request.setAttribute("MESS", "Rate Success!!");
            }
        } catch (Exception e) {
            if (e.getMessage().contains("duplicate")) {
                try {
                    if (ratingDAO.update(ratingDTO)) {
                        request.setAttribute("MESS", "Rate Success!!");
                    }
                } catch (Exception ee) {
